...
cricket(f1, f2, trig) = s1 : *(44) <: ((aa.clip(0, 1.0): wrap), (aa.clip(1.0, 4.0) : wrap)) :> (+):
               normsin : aa.clip(0.0, 1.0) : *(s2) : *(0.3) * ctVol: *(e)
               with {
                   f = 0.8;
                   sig = phasor(f);
...
water = no.noise : abs : ba.sAndH(no.sparse_noise(250) : abs : >(0)) :
        *(1200) : +(400) : os.osc : fi.resonhp(850, 15, 1) : *(0.008) * ctVol;

hum = no.noise : fi.resonlp(800, 1, 0.08) * ctVol;

critters(freqs) = freqs : prod(i, ba.count(freqs), phasor: normcos):>
           fi.resonhp(20, 1, 1) : *(os.osc(0.01)): *(0.025) * ctVol;
...
frog(l, f, trig) = out
    with {
        ...
        vcf(s, res, f) = s : fi.resonbp(f, res, 1): fi.resonlp(1400, 1, 1) * ctVol;
    };
...
process = insects, background : ro.interleave(2, 2) : par(i, 2, sum(j, 2, _))
    with {
        ...
        background = water + hum +
                     ...
                     critters((2134, 4.279, 20.4, 15.5)) : *(0.4) * ctVol <: (_, _);
    };
